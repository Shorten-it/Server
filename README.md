...
┌──────────────────────────────────────────────────────────┐
│                    Nginx (Port 80)                        │
│              API Gateway · least_conn LB                  │
└─────────┬────────────────────────────────┬───────────────┘
          │                                │
   POST/DELETE (쓰기)               GET (읽기/리다이렉트)
          │                                │
   ┌──────▼──────┐                 ┌───────▼───────┐
...
- **해결**: Probabilistic Early Expiration 적용 — TTL 만료 전 확률적으로 캐시를 미리 갱신하여 동시 DB 조회를 원천 차단
- **결과**: TTL 만료 시점의 DB 부하 스파이크 제거, 단일 인스턴스에서도 동시 요청에 의한 DB 경합 방지
